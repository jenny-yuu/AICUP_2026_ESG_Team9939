# ============================================================
# VeriPromise ESG M2 - MultiTask Single Model: macbert × 3 Seed × 5-fold
# ============================================================
# 整合..的 electra 程式碼架構：
#
...
# ✅ Stratify on verification_timeline（同..）
# ============================================================
...
# ── 訓練設定（..版本）─────────────────────────────────
MAX_LEN    = 384    # ..用 384（比舊版 256 更長）
EPOCHS     = 8      # ..用 8
LR         = 2e-5
USE_FGM    = True   # 全任務 FGM（..：所有任務）
USE_AMP    = True   # 混合精度（..：使用）
...
SEVEN_MODELS = {
    "macbert": ("hfl/chinese-macbert-base", 16),  # single model version
}
...
# ============================================================
# 4. 每個任務的 Metadata 格式（你的前處理 + ..的架構）
# ============================================================
# ..架構：MultiTaskModel + mean pooling + AMP + joint loss
...
class MultiTaskModel(nn.Module):
    ...
    def _pool(self, ids, mask):
        """Attention-weighted mean pooling（..版本）"""
...
def decode_probs(probs, n_items):
    """
    ..版本 decode：hard rule post-processing
...
def train_one_model(model_short, model_path, batch_size):
    ...
    # 累積器（..版本：跨 seed×fold 加總 test，跨 seed 加總 oof）
    ...
    # 用 verification_timeline 做 stratify（..版本）
...
print(f"\n{'#'*120}")
print("M2 單模型開始：macbert MultiTaskModel × 3Seed × 5fold")
...
print()
print("="*80)
print("✅ 單模型 macbert 訓練完成。")
print("📌 這份程式只負責產生 /content/M2_pkl/M2_macbert.pkl")
...
║  ✅ 4任務 loss 加總，互相正則化                                          ║
║  ✅ MAX_LEN=384（更長上下文）                                            ║
║  ✅ 單模型 macbert × 3 seeds × 5 folds → 1 pkl                           ║
